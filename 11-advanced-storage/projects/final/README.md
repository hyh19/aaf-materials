...
4. 启动应用：
   ...
   此命令将在模拟器上启动应用，您应该会看到食谱查找应用的主界面。

成功运行应用后，您可以使用以下命令确认应用是否正在前台运行：

```bash
adb shell "dumpsys window | grep -E 'mCurrentFocus|mFocusedApp'" | cat
```

此命令应显示 `com.kodeco.recipefinder/.MainActivity` 在当前焦点中。
...
3. 启动模拟器（使用只读模式避免多实例问题）：
   ...
   如果不确定可用的模拟器名称，可以使用以下命令列出所有可用的模拟器：
...
4. 清理并重新构建项目（确保干净的构建）：
   ...
   注意：构建过程中可能会看到一些警告，如 KSP 版本与 Kotlin 版本不匹配的警告。这些警告不会影响应用的安装和运行，可以安全忽略。当看到 `Installed on 1 device.` 消息时，表示安装已成功。
...
完成任一方式的步骤后，应用都会在模拟器中启动并运行。如果遇到问题，请参考下面的"常见问题解决"章节。
...
这是由于 KSP（Kotlin Symbol Processing）插件版本（1.9.0-1.0.13）与项目使用的 Kotlin 版本（1.9.10）不完全匹配导致的。这个警告通常不影响应用的构建和运行，可以安全忽略。

如果您想消除这些警告，可以考虑以下两种解决方案：

1. 在 `gradle/libs.versions.toml` 文件中升级 KSP 版本到与 Kotlin 1.9.10 兼容的版本
2. 或降级 Kotlin 版本到 1.9.0 以匹配当前的 KSP 版本
